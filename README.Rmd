...
using the `inject_tracer()` function. The following function includes four
...
can be accessed with the `load_traces` function, which returns a `data.frame`
object (in [`tibble` format](https://tibble.tidyverse.org) with one row for
each parameter from each function call.
...
`trace_package()` function automatically injects tracing code into every
