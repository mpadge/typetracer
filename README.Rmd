...
but the language itself is "absurdly dynamic"[^1], and lacks any way to specify
which types are expected by any expression. The `typetracer` package enables
code to be traced to see what types of parameters are actually passed to R
functions. `typetracer` can trace individual functions or entire packages, as
demonstrated below.
...
The example first assigns a variable `isodate` to the first of the evaluated
values, and then calls the function with `pattern = isodate`. The second
